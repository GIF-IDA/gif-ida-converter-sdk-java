...
public class CSV2NodeTransformer<NodeType extends AbstractNode> extends BasicTransformer<CSVLine<NodeType>, NodeType> {
		...
		if (separatorPos >= 0) {
			...
			try {
				...
				if (methodInvokeObject == null) {
					Class<?> invokeObjectClass = Class
							.forName(AbstractGroupNode.class.getPackage().getName() + "." + objectName);
					methodInvokeObject = (NodeType) invokeObjectClass.newInstance();
					Method setNestedObjectMethod = getMethodDeep(object.getClass(), "set" + objectName,
							invokeObjectClass);
			...
			try {
				if (value.equals("")) {
					...
					if (parts.length == 1) {
						...
						logger.warn("invalid value '" + value + "' for type '" + targetType
				...
				logger.error("could not set '" + key + "' to object " + object.toString()
	...
	protected NodeType doTransform(CSVLine<NodeType> csvLine, ITransformContext context) {
		...
		try {
			...
			logger.error("could not create object of type '" + type.toString()
					+ "' (plugin bug - please report bug of plugin)", e);
